...
const Analytics = () => {
  ...
  const getUniquePlatforms = () => {
    ...
    return [...new Set(reports.filter(report => report.celebrity_name === selectedReport.celebrity_name).map(report => report.platform))];
  };
  const platforms = getUniquePlatforms();
  const currentPlatform = selectedReport?.platform || platforms[0];
  return <div className="container animate-fade-in relative min-h-screen">
      {selectedReport && <div className="mb-6">
          <UpdateReminder selectedReport={selectedReport} />
        </div>}

      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-8">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">My Analytics Hub</h1>
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button className="fixed bottom-8 right-4 rounded-full px-6 shadow-lg flex items-center gap-2 bg-primary hover:bg-primary/90" size="lg">
